...
class DocumentProcessor:
    ...
    async def sync_mongodb_to_vectordb(self, knowledge_base_service, batch_size: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get vector store once for all batches
            vector_store = self.get_vector_store()
            if vector_store is None:
                raise RuntimeError("Vector store is not available (embeddings not configured).")
            ...
            for i in range(0, len(documents), batch_size):
                ...
                for doc in batch:
                    try:
                        ...
                        hash_id = doc.get('hash_id', str(doc.get('_id', '')))
                        submitted_at = doc.get('submission_timestamp', datetime.now().isoformat())
                        ...
                        # Normalise additional_references to a string for metadata
                        if isinstance(additional_references, list):
                            additional_references_str = ",".join(additional_references)
                        else:
                            additional_references_str = additional_references or "None"

                        # Build LangChain Document — add directly to vectordb only,
                        # no new MongoDB insert (documents already exist there).
                        document_content = f"Title: {title}\n\nContent: {content}"
                        base_metadata = {
                            "source": source if source else "Unknown",
                            "title": title,
                            "meta_tags": ",".join(meta_tags) if isinstance(meta_tags, list) else (meta_tags or ""),
                            "author_name": author_name if author_name else "Unknown",
                            "additional_references": additional_references_str,
                        ...
                        for idx, d in enumerate(langchain_docs):
                            d.metadata["id"] = f"{hash_id}_text_{idx}"

                        batch_docs.extend(langchain_docs)
                        ...
                        logging.info(f"Queued document for vectordb: {title}")
                ...
                # Bulk-add the whole batch to vectordb in one call
                if batch_docs:
                    vector_store.add_documents(batch_docs)
                    logging.info(f"Added batch of {len(batch_docs)} chunks to vectordb")
